...
class CrossAttention(nn.Module):
    def __init__(
        ...
        self.to_k = nn.Linear(cond_dim, inner_dim, bias=qkv_bias)
        self.to_v = nn.Linear(cond_dim, inner_dim, bias=qkv_bias)
...
class SwinEncoder(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, down_ratio,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False,
                 fused_window_process=False):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
                BasicLayer(
                    dim=dim*(2**i),
                    input_resolution=(input_resolution[0]//(2**i), input_resolution[1]//(2**i)),
                    depth=depth,
                    num_heads=num_heads,
